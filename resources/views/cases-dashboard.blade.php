@vite('resources/js/scripts/case-serch-script.js')

<x-layout>
    <x-slot:title>
        Gadijumi
    </x-slot:title>

    <div class="p-6 grid grid-cols-1 xl:grid-cols-4 gap-6">
        <div class="flex justify-between xl:col-span-4 bg-white border border-stone-200 rounded-xl px-8 py-5">
            <div class="text-xl font-semibold text-stone-700">
                Gadijumi
            </div>
        </div>
        <div class="xl:col-span-4 xl:row-span-8  bg-white border border-stone-200 rounded-xl px-8 py-6">
            <div class="flex justify-between items-center mb-4">
                <div class="text-stone-700 text-xl font-semibold border-b border-stone-100 pb-2"> Gadijumi </div>

                <div class="relative">
                    <div class="absolute inset-y-0 left-2 flex items-center pointer-events-none">
                        <svg class="w-4 h-4 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-4.35-4.35M17 10a7 7 0 1 0-14 0 7 7 0 0 0 14 0z"/>
                        </svg>
                    </div>

                    <input type="text" id="case_serch" class="pl-8 pr-3 py-1.5 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1"/>
                </div>
            </div>
            <div class="overflow-y-auto max-h-[400px]">
                <table id="case_table" class="min-w-full divide-y divide-stone-200 text-sm text-stone-700">
                    <div class="min-w-full divide-y divide-stone-200 text-sm text-stone-700">
                        <tr>
                            <th class="px-4 py-2 font-medium">ID</th>
                            <th class="px-4 py-2 font-medium">Status</th>
                            <th class="px-4 py-2 font-medium">Ierodas</th>
                            <th class="px-4 py-2 font-medium">Red flag</th>
                            <th class="px-4 py-2 font-medium"></th>
                        </tr>
                    </div>
                    <div>
                        @foreach($case as $case)
                            <tr class="text-center">
                                <td>{{ $case->foreignId }}</td>
                                <td>{{ $case->status }}</td>
                                <td>{{ $case->arrival_ts }}</td>
                                <td>
                                    @foreach($case->risk_flags as $flag)
                                        <p>{{ $flag }}</p>
                                    @endforeach
                                </td>
                                <td class="text-blue-600 underline"><a href="/data/{{ $case->foreignId }}">detaļas</a></td>
                            </tr>
                        @endforeach
                    </div>
                </table>
            </div>
        </div>
    </div>
</x-layout>
